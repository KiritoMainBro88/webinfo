...
const specialCharRegex = /[!@#$%^&*(),.?":{}|<>]/; // Add/remove characters as needed
...
router.post('/register', async (req, res) => {
    ...
    let passwordErrors = [];
    if (password.length < 6) { // Keep min length check (or increase if desired)
...
router.post('/forgot-password', async (req, res) => {
    ...
    try {
        ...
        const resetUrl = `${configuredFrontendUrl}/reset-password.html?token=${resetToken}`;
