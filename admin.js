console.log("Admin script version 1.2.2 running!"); // Update log

// --- DEFINE BACKEND_URL LOCALLY FOR ADMIN SCRIPT ---
const BACKEND_URL = 'https://webinfo-zbkq.onrender.com';
// This ensures fetchData (defined globally in script.js) can use it when called from admin.js functions.
...
function initializeAdminPanel() {
    ...
    if (typeof fetchData !== 'function' || typeof formatPrice !== 'function') {
         ...
         // Display an error message to the user in the admin panel
         if(adminContent) {
            adminContent.innerHTML = '<p class="error" style="padding: 2rem; text-align: center;">Critical Error: Admin functions cannot load. Please check the browser console.</p>';
            adminContent.style.display = 'block'; // Make sure the error is visible
         }
         if(authCheckDiv) authCheckDiv.style.display = 'none'; // Hide the auth check message
         return; // Stop further initialization
    }

    // Load data and setup forms based on which admin page we are on
    if (document.getElementById('category-management')) {
        // Load categories first, then products if the product form exists
        loadCategories().then(() => {
            ...
            // Decide if you still want to try loading products or setting up forms
            if (document.getElementById('product-management')) {
                 loadProducts(); // Attempt to load products anyway
            }
        });
    } else if (document.getElementById('product-management')) {
        // If only product management exists (unlikely but possible), load categories for dropdown
        loadCategories().then(() => loadProducts()).catch(e => console.error("Error loading initial product data:", e));
    }

    // Setup forms - Attaches listeners, safe now
    setupAdminForms();
}
...
async function loadCategories() {
    ...
    try {
        // Uses global fetchData, which now uses BACKEND_URL defined in *this* file's scope if called from here
        // or the global one if called from script.js scope.
        ...
        if (categoryListDiv) {
            ...
            if (!categories || categories.length === 0) {
                ...
                categories.forEach(cat => {
                    ...
                    li.dataset.categoryId = cat._id; // Add data-id for easier selection
                    li.innerHTML = `
        ...
        const errorMsg = `<p class="error" style="color: var(--danger-color);">Error loading categories: ${error.message}</p>`;
        if (categoryListDiv) categoryListDiv.innerHTML = errorMsg;
        if (categorySelect) categorySelect.innerHTML = '<option value="">-- Error Loading --</option>';
        throw error;
    }
}
...
async function loadProducts() {
     ...
     try {
         ...
         productListDiv.innerHTML = `<p class="error" style="color: var(--danger-color);">Error loading products: ${error.message}</p>`;
         throw error;
     }
 }
...
function createProductTable(products) {
    ...
    products.forEach(prod => {
        ...
        tr.dataset.productId = prod._id; // Add data-id
        // Use global formatPrice (check if it exists)
...
function setupAdminForms() {
    ...
    if (addCategoryForm && editCategoryIdField && cancelEditCategoryBtn) {
        addCategoryForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                // Reload categories and potentially products
                await loadCategories(); // await ensures category list/dropdown is updated
                if (document.getElementById('product-management')) {
                    await loadProducts(); // Reload products as category changes might affect them
                }
    ...
    if (addProductForm && editProductIdField && cancelEditProductBtn) {
         addProductForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            const formData = new FormData(addProductForm); const productData = Object.fromEntries(formData.entries()); const isEditing = !!editProductIdField.value; const productId = editProductIdField.value;
     ...
     if (addBalanceForm) {
        addBalanceForm.addEventListener('submit', async (e) => {
            e.preventDefault(); const userIdInput = document.getElementById('balance-user-id').value.trim(); const amount = document.getElementById('balance-amount').value; const button = addBalanceForm.querySelector('button[type="submit"]');
            ...
            try {
                ...
                displayMessage('balance-message', result.message || 'Balance updated!', true); addBalanceForm.reset();
    ...
    } else if (document.getElementById('balance-management')) { console.warn("Balance form not found."); }
...
function handleCategoryActions(e) {
    ...
    if (target.classList.contains('delete-cat-btn') && catId) {
        ...
        const catName = listItem ? listItem.querySelector('span:first-child')?.textContent.split(' (Order:')[0].trim() : catId; // Get name for confirm message
        if (confirm(`DELETE Category "${catName}"? Check products first!`)) {
            fetchData(`/categories/${catId}`, { method: 'DELETE' })
                .then(() => {
                    displayMessage('cat-message', 'Category deleted.', true);
                    loadCategories();
                    if (document.getElementById('product-management')) {
                         loadProducts(); // Reload products as category is gone
                    }
                })
...
async function handleProductActions(e) {
     ...
     if (target.classList.contains('delete-btn') && prodId) {
         ...
         const prodName = tableRow ? tableRow.querySelector('td:nth-child(2)')?.textContent.trim() : prodId; // Get name for confirm
         if (confirm(`DELETE Product "${prodName}"?`)) {
             try {
                 await fetchData(`/products/${prodId}`, { method: 'DELETE' });
                 displayMessage('product-message', 'Product deleted.', true); loadProducts();
             } catch (error) { displayMessage('product-message', `Error deleting: ${error.message}`, false); }
         }
     }
         ...
         try {
             ...
             editProductIdField.value = product._id; document.getElementById('product-name').value = product.name || ''; document.getElementById('product-category').value = product.category?._id || ''; document.getElementById('product-price').value = product.price ?? ''; document.getElementById('product-original-price').value = product.originalPrice ?? ''; document.getElementById('product-image').value = product.imageUrl || ''; document.getElementById('product-tags').value = product.tags?.join(', ') || ''; document.getElementById('product-stock').value = product.stockStatus || 'in_stock'; document.getElementById('product-order').value = product.displayOrder ?? 0; document.getElementById('product-description').value = product.description || ''; addProductForm.querySelector('h3').textContent = 'Edit Product'; cancelEditProductBtn.style.display = 'inline-block'; addProductForm.scrollIntoView({ behavior: 'smooth', block: 'start' });
