...
const BACKEND_URL = 'https://webinfo-zbkq.onrender.com'; // Your actual backend URL
const BACKUP_MODE = false; // Disable backup mode to use real API
...
function createProductCardElement(product, includeBuyButton = false, categorySlug = null) { 
    ...
    const badgeType = product.badgeType || "best-seller";
    const badgeColor = product.badgeColor || ""; 
    const badgeStyle = badgeColor ? `style="background-color:${badgeColor}"` : "";
    
    // Get country/brand badge if available
    const brandBadge = product.brandBadge || "VN";
    const brandBadgeColor = product.brandBadgeColor || "";
    const brandBadgeStyle = brandBadgeColor ? `style="background-color:${brandBadgeColor}"` : "";

    // Create the card content matching the image design
    card.innerHTML = `
        <div class="product-image-placeholder">
            <img src="${product.imageUrl || 'https://placehold.co/400x225/191c26/e9ecef?text=No+Image'}" alt="${escapeHtml(product.name)}">
...
async function fetchAndUpdateUserInfo() { 
    ...
    try {
        ...
        // Only clear localStorage if it's an authentication error (401/403)
        // or if we received invalid data from the server
        if (error.status === 401 || error.status === 403 || error.message === "Received invalid user data") {
        ...
        // For other errors (network/timeout), keep the user logged in
            ...
            // Use cached values from localStorage as fallback
            const cachedUsername = localStorage.getItem('username');
            const cachedBalance = localStorage.getItem('balance');
            const cachedIsAdmin = localStorage.getItem('isAdmin') === 'true';
            
            if (cachedUsername) {
                updateUserStatus(true, cachedUsername, cachedIsAdmin);
                updateSidebarUserArea(true, cachedUsername, cachedBalance ?? 0);
            }
        }
    }
}
...
function createProductCard(product) {
    ...
    // Get country/brand badge
    const brandBadge = product.brandBadge || "VN";
    const brandBadgeColor = product.brandBadgeColor || "";
    const brandBadgeStyle = brandBadgeColor ? `style="background-color:${brandBadgeColor}"` : "";

    // Calculate discount badge if original price exists
