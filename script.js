...
async function fetchData(endpoint, options = {}) {
    ...
    const tempUserId = localStorage.getItem('userId'); // Get ID for the conceptual check
     options.headers = {
        'Content-Type': 'application/json',
        ...options.headers,
        // IMPORTANT: Replace X-Temp-UserId with real Authorization: Bearer token later
...
function setupPurchaseModals() {
    const productArea = document.getElementById('dynamic-product-area'); const purchaseModal = document.getElementById('purchase-modal'); const confirmModal = document.getElementById('confirm-modal'); const processingModal = document.getElementById('processing-modal'); const purchaseForm = document.getElementById('purchase-form'); if (!productArea || !purchaseModal || !confirmModal || !processingModal || !purchaseForm) { console.warn("One or more essential elements for purchase flow not found."); return; } let currentPurchaseData = {}; productArea.addEventListener('click', (e) => { if (e.target.classList.contains('product-buy-btn')) { e.preventDefault(); if (!localStorage.getItem('userId')) { showMessageModal('Lỗi', 'Vui lòng đăng nhập để mua hàng!', 'error'); return; } const card = e.target.closest('.product-card'); if (!card) return; const productId = card.dataset.productId; const productName = card.dataset.productName || 'Sản phẩm không rõ'; const productPrice = parseInt(card.dataset.productPrice, 10) || 0; const stockStatus = card.dataset.stockStatus || 'in_stock'; if (stockStatus === 'contact' || stockStatus === 'check_price') { let contactMsg = `Vui lòng liên hệ Admin qua Discord hoặc Facebook để ${productName === 'Bán Robux Chính Hãng' ? 'xem bảng giá Robux' : 'biết chi tiết dịch vụ này'}.`; showMessageModal('Thông tin', contactMsg, 'info'); return; } if (stockStatus === 'out_of_stock') { showMessageModal('Thông báo', `Sản phẩm "${productName}" hiện đã hết hàng.`, 'warning'); return; } currentPurchaseData = { productId, productName, productPrice }; document.getElementById('purchase-item-name').textContent = productName; document.getElementById('purchase-item-id').value = productId; document.getElementById('purchase-total-price').textContent = formatPrice(productPrice); purchaseForm.reset(); showModal('purchase-modal'); } }); purchaseForm.addEventListener('submit', (e) => { e.preventDefault(); if (!document.getElementById('purchase-confirm-details').checked) { showMessageModal('Lỗi', 'Bạn cần xác nhận đã cung cấp đúng thông tin.', 'warning'); return; } hideModal('purchase-modal'); document.getElementById('confirm-modal-text').textContent = `Bạn sẽ mua "${currentPurchaseData.productName}" với giá ${formatPrice(currentPurchaseData.productPrice)}?`; showModal('confirm-modal'); }); document.getElementById('confirm-agree-btn')?.addEventListener('click', () => { hideModal('confirm-modal'); showModal('processing-modal'); const formData = new FormData(purchaseForm); const purchaseDetails = Object.fromEntries(formData.entries()); console.log("Attempting Purchase:", { ...currentPurchaseData, ...purchaseDetails }); let currentBalance = parseInt(localStorage.getItem('userBalance') || '0', 10); let itemPrice = currentPurchaseData.productPrice; let isSuccess = false; if (currentBalance >= itemPrice) { let newBalance = currentBalance - itemPrice; localStorage.setItem('userBalance', newBalance.toString()); console.log(`DEMO: Balance updated. Old: ${formatPrice(currentBalance)}, New: ${formatPrice(newBalance)}`); updateSidebarUserArea(true, localStorage.getItem('username')); isSuccess = true; } else { console.log(`DEMO: Insufficient balance. Needed: ${formatPrice(itemPrice)}, Has: ${formatPrice(currentBalance)}`); isSuccess = false; } setTimeout(() => { hideModal('processing-modal'); if (isSuccess) { showMessageModal('Thành công!', 'Mua vật phẩm thành công! Vui lòng kiểm tra tài khoản của bạn.', 'success'); } else { showMessageModal('Thất bại!', 'Số dư không đủ hoặc đã xảy ra lỗi. Vui lòng nạp thêm hoặc liên hệ hỗ trợ.', 'error'); } }, 2000); }); document.getElementById('confirm-cancel-btn')?.addEventListener('click', () => { hideModal('confirm-modal'); }); document.querySelectorAll('.modal-overlay .modal-close-btn').forEach(button => { button.addEventListener('click', (e) => { const modal = e.target.closest('.modal-overlay'); if (modal) { hideModal(modal.id); } }); }); document.getElementById('message-ok-btn')?.addEventListener('click', () => hideModal('message-modal')); document.getElementById('notification-ok-btn')?.addEventListener('click', () => hideModal('notification-modal')); document.querySelector('#purchase-modal .modal-cancel-btn')?.addEventListener('click', () => hideModal('purchase-modal')); document.querySelectorAll('.modal-overlay').forEach(overlay => { overlay.addEventListener('click', (e) => { if (e.target === overlay && overlay.id !== 'processing-modal' && overlay.id !== 'confirm-modal') { hideModal(overlay.id); } }); });
}
