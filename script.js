console.log("Script version 1.9.7 - Frontend Balance Check & Header Fix"); // Increment version
...
let purchaseModal, purchaseForm, purchaseItemId, purchaseItemName, purchaseTotalPrice, purchaseMessage, purchaseCloseBtn, purchaseCancelBtn, purchaseItemPriceInput; // Added purchaseItemPriceInput
...
function setupPurchaseModalListeners() {
    // Elements are now global (purchaseModal, purchaseForm, etc.)
    if (!purchaseModal || !purchaseForm || !purchaseCloseBtn || !purchaseCancelBtn || !purchaseMessage || !purchaseItemPriceInput) {
        return; // Exit silently if elements aren't found (e.g., not on shopping page)
    }
    ...
    purchaseForm.addEventListener('submit', async (e) => {
        ...
        purchaseMessage.textContent = 'Đang xử lý...';
        purchaseMessage.className = 'auth-message'; // Reset class
        ...
        const itemPrice = parseFloat(purchaseItemPriceInput.value); // Get price from hidden input

        if (isNaN(itemPrice)) {
             console.error("Item price is not a valid number in the modal.");
             purchaseMessage.textContent = 'Lỗi: Giá sản phẩm không hợp lệ.';
             purchaseMessage.className = 'auth-message error';
             return; // Stop processing
        }

        console.log(`Checking balance: User Balance=${currentBalance}, Item Price=${itemPrice}`); // Debug log
        ...
        try {
            // ** SIMULATE PURCHASE (Replace with actual backend call) **
            console.log("Simulating purchase with data:", purchaseData);
            ...
            // Simulate success
            purchaseMessage.textContent = 'Mua hàng thành công! Kiểm tra lịch sử mua hàng.';
            ...
            console.log("Simulated purchase successful for item ID:", purchaseData.itemId);
             ...
             // This needs to run *after* the backend confirms success and deducts balance
...
function initializePage() {
    console.log(`Initializing page (v1.9.7)...`); // Update version
    ...
    setupDropdownActions(); // Deposit/History links etc.
    setupProfessionalAnimations(); // Run animations last
    // Page specific init
