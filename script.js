console.log("Script version 1.3 running!"); // Updated version
...
function setupHeaderScrollEffect() {
    ...
    checkScroll(); window.addEventListener('scroll', checkScroll, { passive: true }); window.addEventListener('resize', checkScroll);
}
...
function updateSidebarUserArea(isLoggedIn, username = 'Khách') {
     ...
     // Get hypothetical balance for demo
     const demoBalance = parseInt(localStorage.getItem('userBalance') || '0', 10);

     if (isLoggedIn) {
        usernameSpan.textContent = username;
        balanceSpan.textContent = formatPrice(demoBalance); // Show demo balance
...
function setupDropdownActions() {
    ...
    // Buy listeners moved to setupPurchaseModals
}
...
function setupPurchaseModals() {
    ...
    buyButtons.forEach(button => {
        button.addEventListener('click', (e) => {
            ...
            purchaseForm.reset(); // Reset fields
    ...
    purchaseForm.addEventListener('submit', (e) => {
        ...
        // Use stored data for confirmation text
        document.getElementById('confirm-modal-text').textContent = `Bạn sẽ mua "${currentPurchaseData.productName}" với giá ${formatPrice(currentPurchaseData.productPrice)}?`;
        ...
        setTimeout(() => {
            ...
            if (isSuccess) {
                showMessageModal('Thành công!', 'Mua vật phẩm thành công! Vui lòng kiểm tra tài khoản của bạn.', 'success');
                 ...
                 showMessageModal('Thất bại!', 'Số dư không đủ hoặc đã xảy ra lỗi. Vui lòng nạp thêm hoặc liên hệ hỗ trợ.', 'error');
            }
...
function initializePage() {
    console.log("Initializing page (v1.3)..."); // Update log
    updateYear();
    ...
    setupThemeToggle();
    setupLanguageToggle();
    setupProfessionalAnimations(); // GSAP scroll/load animations
    setupAdminPanel();
    ...
    setupPurchaseModals(); // Purchase flow
